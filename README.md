...
Копия доски — снимок состояния с указателем на исходное сообщение (кнопка
`❌ Закрыть` несёт `chat_id` и `message_id`). Нажатие в копии обновляет исходную
доску и гасит копию: `editMessageText` требует прислать клавиатуру целиком, а
прочитать чужое сообщение бот не умеет — поэтому копия и есть снимок состояния.

Копия приходит в трёх случаях:

* **🙇 на своём же ресурсе** — в личку, вместе с шуткой про зеркало;
* **🙇 на чужом ресурсе** — держателю в личку: просьбу освободить можно выполнить
  прямо оттуда, не возвращаясь в чат;
* **нажатие на чужой занятый ресурс** — нажавшему, как подтверждение: в чате
  ничего не меняется, ресурс освобождает уже нажатие в копии.

Подтверждение приходит [эфемерным сообщением](https://core.telegram.org/bots/api#ephemeral-messages-and-commands)
(Bot API 10.2): оно живёт в том же чате и в той же теме форума, но видит его
только нажавший — идти в личку не нужно, и остальным чат не засоряется. Право
ответить эфемерно даёт `callback_query_id` и живёт 15 секунд, поэтому бот
отправляет копию сразу. У эфемерного сообщения `message_id` всегда `0`, так что
правится оно своим методом — `editEphemeralMessageText` с `receiver_user_id` и
`ephemeral_message_id`.

Встроенного диалога «точно?» у Bot API нет: `answerCallbackQuery` умеет только
всплывашку с одной кнопкой OK. Поэтому подтверждение и сделано отдельным
сообщением с кнопками.

Копии отправляются через [rich message](https://core.telegram.org/bots/api#rich-messages)
(`sendRichMessage` c `rich_message.html`). Кнопки копии живут прямо в разметке — `<tg-button-row>` с
